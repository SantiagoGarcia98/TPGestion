...
public:
    ...
    /// LISTADOS: línea 328 del .cpp
    void listarVentas(); /// OK
    void mostrarVentaCompleta(int idVenta); /// OK, muestra todo el detalle de la venta
    void listarVentaPorFecha(); /// OK, ordenada por fecha de más reciente a antigua
    void listarVentaPorMonto(); /// OK, ordenadas por monto total de manera descendente

    /// CONSULTAS
    void consultarVentasPorFechas(); /// OK, por rango de fechas
    void consultarVentaPorCliente(); /// OK, por cliente (dni podrías ser)
    void consultarVentaPorVendedor(); /// Por vendedor (legajo podría ser)
    void consultarVentaPorAutoVendido(); /// OK, Por vehiculo vendido
};
